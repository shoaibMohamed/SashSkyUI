// Outlet id's
// Sash Cafe = 1
// Sash Sky = 2
// Sash Express =3
// Sash Urban = 4
// The id's start with 1 digit and then 2 digits for every level
export const SASH_SKY_MENU = [
    {   
        id:1,
        name: "Soups",
        description: "Enjoy our fresh soups",
        availableOutlets:[2],
        dishes:[
            {   
                id:101,
                isSelected:false,
                name:"Cream Of Chicken",
                description:"A classic go to option for chickentarians",
                price:"290",
                diet:"N",
                availableOutlets:[2]
            },
            {   
                id:102,
                isSelected:false,
                name:"Lemon And Chicken",
                description:"Balanced blend with chinese spices",
                price:"280",
                diet:"N",
                availableOutlets:[2]
            }
        ]
    },
    {   
        id:2,
        name: "Pan Asian",
        description: "Enjoy our pan asian appetizers",
        availableOutlets:[2],
        dishes:[
            {
                id:201,
                isSelected:false,
                name:"China Town Chilly Paneer",
                description:"",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:202,
                isSelected:false,
                name:"Crispy Corn",
                description:"",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:203,
                isSelected:false,
                name:"Honey Crispy Potatoes",
                description:"",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:204,
                isSelected:false,
                name:"Chilly Broccoli",
                description:"",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:205,
                isSelected:false,
                name:"Thai Spring Rolls",
                description:"Vegetables rolls with thai flavours, served with sweet chili sauce",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:206,
                isSelected:false,
                name:"Mushroom Pepper Fry",
                description:"",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:207,
                isSelected:false,
                name:"Babycorn In Chilli Shezwan Sauce",
                description:"",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:208,
                isSelected:false,
                name:"Manchurain Balls",
                description:"A go to option for vegetarians",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:209,
                isSelected:false,
                name:"Hong Kong Chilli Chicken",
                description:"",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:210,
                isSelected:false,
                name:"Dragon Chicken Lolipop",
                description:"",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:211,
                isSelected:false,
                name:"Thighs from Hell",
                description:"A must try option for spicy lovers",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:212,
                isSelected:false,
                name:"Chicken Spring Rolls",
                description:"Attempt to recreate the old school canteen rolls, give it a try",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:213,
                isSelected:false,
                name:"Crispy Seaseme Chicken",
                description:"Tossed in sesame oil and vegetables. A perfect blend",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:214,
                isSelected:false,
                name:"Lemon Chicken",
                description:"Chili and soury crispy slices , served with thai salad and sweet chili sauce",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:215,
                isSelected:false,
                name:"Fiery Fish",
                description:"",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:216,
                isSelected:false,
                name:"Prawns In Pepper Sauce",
                description:"",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
        ]
    },
    {
        id:3,
        name: "TandoorPot Oven",
        description: "Enjoy our freshly prepared tandoori",
        availableOutlets:[2],
        dishes: [
            {
                id:301,
                isSelected:false,
                name:"Hara Bhara Kebab",
                description:"shallow fried vgetable patty flavored with sourthern spices",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:302,
                isSelected:false,
                name:"Kurkuri Cheese Kebab",
                description:"Peas and aloo patty filled with mouth melting cheese, served with mint chutney",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:303,
                isSelected:false,
                name:"Dahi ke Sholey",
                description:"A better version of dahi kebab",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:304,
                isSelected:false,
                name:"Veg Sheek",
                description:"All time favourite for vegetarians",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:305,
                isSelected:false,
                name:"Achari Paneer Tikka",
                description:"Punjabi pickled marinated cottage cheese, roasted in clay pot oven",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:306,
                isSelected:false,
                name:"Malai Broccoli",
                description:"Creamy marinated florets flavored with whole garam masala, roasted in clay pot oven",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:307,
                isSelected:false,
                name:"Mirchi Bhare Aluu",
                description:"Potatoes filled khoya, chilies and nuts, marinated in spicy masala and roasted in clay pot oven",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:308,
                isSelected:false,
                name:"Afghan Se Bulai Chap",
                description:"Soya chaap marinated with afghan flavours and roasted with smokey flavours",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:309,
                isSelected:false,
                name:"South Delhi Aunty Chap",
                description:"Masala marinated chaap with smokey flavours, served with green chutney",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:310,
                isSelected:false,
                name:"Jain Special Mangodi",
                description:"overnight soaked moondal, deepfried with chillies and nuts",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:311,
                isSelected:false,
                name:"Punjabi Kukkad",
                description:"Spiced Chicken Marinated in Yougurt and Masalas, roasted in clay pot ove, finished with butter and served with tangy onion salad and mint chutney",
                price:"299/550",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:312,
                isSelected:false,
                name:"Ancestral Chicken Tikka",
                description:"The world fav indian chicken prepration made with homemade spice marinade",
                price:"325",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:313,
                isSelected:false,
                name:"Cheesy Malai Tikka",
                description:"Creamy suclent chicken roasted in claypot oven and served with tangy salad and chutney",
                price:"345",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:314,
                isSelected:false,
                name:"Tangdi Kebab",
                description:"Chicken Drumsticks marinated with chef's special masala cooked to prefection",
                price:"325",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:315,
                isSelected:false,
                name:"Tandoori Wings",
                description:"Smokey Flavoured, marinated in inidan spices and yougurt and grilled to perfection, will blow your mind away",
                price:"299/399",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:316,
                isSelected:false,
                name:"Ajwaini Fish Tikka",
                description:"All time Classic Fish Tikka, laced with robust spices",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:317,
                isSelected:false,
                name:"Tandoori Fish Of The Day",
                description:"Please ask for chef",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:318,
                isSelected:false,
                name:"Achari Jhingha",
                description:"Pickled Flavour marinated Prawns, served with salad and chutney",
                price:"",
                diet:"N",
                availableOutlets:[2]
            }
        ]
    },
    {
        id:4,
        name: "Dimsen",
        description: "Steamed, Fried And Tandoori",
        availableOutlets:[2],
        dishes: [
            {
                id:401,
                isSelected:false,
                name:"Four Mushrooms",
                description:"",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:402,
                isSelected:false,
                name:"Mix Veg",
                description:"",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:403,
                isSelected:false,
                name:"Veg Kheema",
                description:"",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:404,
                isSelected:false,
                name:"Chilli And Paneer",
                description:"",
                price:"",
                diet:"V",
                availableOutlets:[2]
            }, 
            {
                id:405,
                isSelected:false,
                name:"Chicken And Cheese",
                description:"",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:406,
                isSelected:false,
                name:"Oyester Chicken",
                description:"",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:407,
                isSelected:false,
                name:"Jalapeno And Chicken",
                description:"",
                price:"",
                diet:"N",
                availableOutlets:[2]
            }
        ]
    },
    {   
        id:5,
        name: "Continental",
        description: "Continental appetizers",
        availableOutlets:[2],
        dishes:[
            {   
                id:501,
                isSelected:false,
                name:"Peri Peri Fries",
                description:"Served with garlic mayonnaise",
                price:"180",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:502,
                isSelected:false,
                name:"Garlic Herbed Bun",
                description:"Inhouse bun topped with herbed garlic butter and seasoned with peri peri seasoning",
                price:"180",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:503,
                isSelected:false,
                name:"Cheesy Garlic Bread",
                description:"Inhouse bun topped with cheese and herbed garlic butter and seasoned with peri peri seasoning",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:504,
                isSelected:false,
                name:"Crumfried Mushroom",
                description:"Stuffed with cheese and jalapenos and served with creamy dip",
                price:"285",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:505,
                isSelected:false,
                name:"Jalapeno Poppers",
                description:"Mouth melting balls with pepper mayonnaise",
                price:"215",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:506,
                isSelected:false,
                name:"Loaded Nachos",
                description:"Golden nachos with melted cheese and mexican salsa",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:507,
                isSelected:false,
                name:"Cheesy Onion Rings",
                description:"",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:508,
                isSelected:false,
                name:"Veg Cutlets",
                description:"",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:509,
                isSelected:false,
                name:"Paneer Tikka Popcorn",
                description:"Different version of popcorn, a must try",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:510,
                isSelected:false,
                name:"Veg Quesadilla",
                description:"Mexican flat bread stuffed with vegetables and cheese, grilled on a flat tawa",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {
                id:511,
                isSelected:false,
                name:"Chicken Tikka Popcorn",
                description:"Tandoori flavoured popcorn, served with garlic mayo",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:512,
                isSelected:false,
                name:"Cheesy Chicken Balls",
                description:"",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:513,
                isSelected:false,
                name:"Sash Special Crispy Chicken",
                description:"",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:514,
                isSelected:false,
                name:"Chicken Tacos",
                description:"Crispy tortilla topped with shredded chicken and bell peppers with salsa and cheese",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:515,
                isSelected:false,
                name:"Cajun Crispy Fish",
                description:"",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:516,
                isSelected:false,
                name:"Chettinadu Prawns Fritters",
                description:"Classic one with a twist",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:517,
                isSelected:false,
                name:"Basil Garlic Prawns",
                description:"A classic one",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:518,
                isSelected:false,
                name:"Kalamari Rings",
                description:"Battered fry squid rings dusted with chili seasoning and served with garlic aioli",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {
                id:519,
                isSelected:false,
                name:"Mediterrian Platter",
                description:"Bouqet of all lebanese pockets served with varities of dips, pita and pickled vegetables",
                price:"",
                diet:"N",
                availableOutlets:[2]
            }
        ]
    },
    {   
        id:6,
        name: "Pizza",
        description: "Our in house pizzas",
        availableOutlets:[2],
        dishes:[
            {   
                id:601,
                isSelected:false,
                name:"Nepolitan Margherita",
                description:"Pizza made with marinara sauce, fresh mozzarella cheese and basil",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {   
                id:602,
                isSelected:false,
                name:"Farmhouse Rocket",
                description:"Baby corn, Mushroom, Brocolli, Sundried Tomato, Onion, Lettuce and topped with chili pickle",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {   
                id:603,
                isSelected:false,
                name:"Double Fungi",
                description:"Creamy mushroom sauce with basil and chili",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {   
                id:604,
                isSelected:false,
                name:"Peppy Paneer",
                description:"Paneer Tikka with onion, tomatoes and sliced chili",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {   
                id:605,
                isSelected:false,
                name:"Basil Chicken",
                description:"Chicken Chunks marinated with basil and jalapenoes, topped with fresh mozzarella cheese",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {   
                id:606,
                isSelected:false,
                name:"Meatballs and grilled vegetables",
                description:"Chciken meatballs, with marinara sauce and topped with 2 typs of cheese",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {   
                id:607,
                isSelected:false,
                name:"MOM",
                description:"Loaded with meat and caramelized onion and chili pickle",
                price:"",
                diet:"N",
                availableOutlets:[2]
            }
        ]
    },
    {   
        id:7,
        name: "Burger",
        description: "Fresh Burgers",
        availableOutlets:[2],
        dishes:[
            {   
                id:701,
                isSelected:false,
                name:"Vegetable Patty Burger",
                description:"Served with fries,salad and garlic mayo",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {   
                id:702,
                isSelected:false,
                name:"Crispy Cottage Cheese",
                description:"Served with fries,salad and peri peri mayo",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {   
                id:703,
                isSelected:false,
                name:"Crispy chicken ",
                description:"Served with fries,salad and tandoori mayo",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {   
                id:704,
                isSelected:false,
                name:"Kheema burger",
                description:"Served with tangy salad and mint chutney",
                price:"",
                diet:"N",
                availableOutlets:[2]
            }
        ]
    },
    {   
        id:8,
        name: "Pasta",
        description: "Fresh Prepared Pasta",
        availableOutlets:[2],
        dishes:[
            {   
                id:801,
                isSelected:false,
                name:"Classic Penne",
                description:"In white/red/pink sauce (Veg/Chicken/Prawn)",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {   
                id:802,
                isSelected:false,
                name:"Spaghetti",
                description:"In white/red/pink sauce (Veg/Chicken/Prawn)",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {   
                id:803,
                isSelected:false,
                name:"Aglio-o-olio",
                description:"In white/red/pink sauce (Veg/Chicken/Prawn)",
                price:"",
                diet:"V",
                availableOutlets:[2]
            },
            {   
                id:804,
                isSelected:false,
                name:"Chicken Meatballs",
                description:"In white/red/pink sauce",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {   
                id:805,
                isSelected:false,
                name:"Ratatoulie Lasagna",
                description:"",
                price:"",
                diet:"N",
                availableOutlets:[2]
            },
            {   
                id:806,
                isSelected:false,
                name:"Minced Chicken Lasagna",
                description:"",
                price:"",
                diet:"N",
                availableOutlets:[2]
            }
        ]
    }
]
